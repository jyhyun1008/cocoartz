...
function landLoader(){
    ...
    landload.load('/assets/models/plant/BasicTree.gltf', function (gltf) {
        ...
        itemBox = new THREE.BoundingBoxHelper( item );
        itemBox.update(item);
        itemBox.box.min.x -= 0.1;
        itemBox.box.max.x += 0.1;
        scene.add(itemBox);
...
function mousemove(e)
{
    ...
    if( iIsDrag  && (timer > timerIndex * 216)) {
        ...
        if( iIsDrag == 1 ) {
            ...
            console.log(e.clientX, bb.left, iStartX, iCircleX);
            ...
            if ((iX-75 != 0 || -iY+75 != 0)){
                ...
                if (angle !== NaN ){
                    my_position.x += 0.2 * Math.cos(angle - my_position.dir);
                    my_position.z -= 0.2 * Math.sin(angle - my_position.dir);
        ...
        if (isFirst == 1 ){
            ...
            walker = setInterval(() => {
                ...
                if ( isPressed ){
                    ...
                    if (e.keyCode==37) { //왼쪽
                        my_position.x -= 0.2* Math.cos(my_position.dir);
                        my_position.z -= 0.2* Math.sin(my_position.dir);
                        my_position.dir -= Math.PI/2;
                    } else if (e.keyCode==38) { //위
                        my_position.x += 0.2* Math.sin(my_position.dir);
                        my_position.z -= 0.2* Math.cos(my_position.dir);
        ...
        if (connectedUsers[i] == userId.name){
            ...
            if ( scene.children[i].itemType == 'avatar' ) {
                ...
                if (my_name == userId.name){
                    ...
                    camera.position.x = cam.prex + timeDiff/200 * (cam.posx - cam.prex);
                    camera.position.y = cam.prey + timeDiff/200 * (cam.posy - cam.prey);
                    camera.position.z = cam.prez + timeDiff/200 * (cam.posz - cam.prez);
